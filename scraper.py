...
def is_similar(text):
    ...
    page_hash = Simhash(text).value

    for old_hash in visited_hashes:
        similarity = bin(page_hash ^ old_hash).count("1")
        if similarity < 5:  # Adjust threshold as needed
...
def tokenize_text(text):
    """Tokenizes text content from a web page."""
...
def get_report():
    """Generates final report for the crawler statistics."""
    print(f"Total Unique Pages: {len(visited_urls)}")
    print(f"Longest Page: {longest_page[0]} with {longest_page[1]} words")
    print("Top 50 Most Common Words:")
    for word, count in sorted(word_counts.items(), key=lambda item: item[1], reverse=True)[:50]:
        print(f"{word}: {count}")

    print("\nSubdomains in ics.uci.edu:")
    for domain in sorted(subdomains.keys()):
